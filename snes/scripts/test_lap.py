"""
Prints the raw lap byte and computed cur_lap each step for 200 frames.
Run:
    docker run -it --rm -v "$(pwd):/code" snes3 \
      -c "cp /code/snes9x_libretro_fixed.so /opt/conda/envs/dev/lib/python3.10/site-packages/retro/cores/snes9x_libretro.so && \
          ...
          /opt/conda/envs/dev/bin/python /code/scripts/test_lap.py"
"""
import sys
import numpy as np
...
print(f"  {'frame':>5}  {'raw_lap':>8}  {'cur_lap (raw-127)':>18}")
print("-" * 40)
...
for i in range(300):
    ...
    if i % 10 == 0:
        print(f"  {i+1:>5}  {raw_lap:>8}  {cur_lap:>18}")
